<x-app-layout>
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('chirps.store') }}">
            @csrf
            <textarea name="message" placeholder="{{ __('O que você está pensando?') }}"
            ...
            <x-primary-button class="mt-4">{{ __('Chirpar') }}</x-primary-button>
        </form>
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div>
                                ...
                                <small class="ml-2 text-sm text-gray-600">
                                    @php
                                        // Get chirp hour
                                        $chirpDateTime = $chirp->created_at;
                                        
                                        // Sets the time zone for Brasilia
                                        $brasiliaTimezone = new DateTimeZone('America/Sao_Paulo');
                                        $chirpDateTime->setTimezone($brasiliaTimezone);
                                        
                                        // Format the date and time in the desired format
                                        $formattedDateTime = $chirpDateTime->format('j M Y, H:i');
                                    @endphp
                                    {{ $formattedDateTime }}
                                </small>
